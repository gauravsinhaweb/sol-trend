...
export const BottomNav: React.FC<BottomNavProps> = ({ className = '' }) => {
    const navItems = [
        ...
        { name: 'Discover', icon: 'compass', active: true },
    ...
    return (
        <nav className={`bg-black border-t border-stone-700 px-6 py-3 ${className}`}>
            <div className="flex items-center justify-around">
                {navItems.map((item) => (
                    <button
                        key={item.name}
                        className={`flex flex-col items-center space-y-1 px-3 py-2 transition-colors ${item.active
                            ? 'text-primary-500'
